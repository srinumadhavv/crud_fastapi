...
async def create_posts(post:schemas.PostCreate,db: Session = Depends(get_db),current_user:int=Depends(oauth.get_current_user)):
...
#order matters while using path parameter as it could reflect in errors video timestamp: 1:50:00
@router.get("/{id}",response_model=schemas.PostOut) # path parameter always returns a string here id will be a string
